...
class Hr extends Controller
{
    ...
    {
        $get_all_leaves = $this->adminModel->get_hod_approved_leaves();
        $data = [
    ...
    {
        $get_all_leaves = $this->adminModel->get_all_leaves();
        $data = [
            'get_all_leaves' => $get_all_leaves
        ];
    ...
    public function hod_approval($id){
        ...
        $update_hod_approved = $this->adminModel->update_hod_approved($id,$hod_approval);
    ...
    public function update_leave_status($id)
    {
        ...
        $leave_detail = $this->adminModel->get_leaves_detail($id);
    ...
    //                 $number_of_days=2;
    //                 $Date = $start_date;
    // $end_date=date('Y-m-d', strtotime($Date. ' + 2 days'));
...
$dateArr = getDateForSpecificDayBetweenDates($start_date,$end_date, 0);
  ...
  $number_of_days = $number_of_days-$count_of_sundays;
    ...
    $salary_detail = $this->userModel->get_salary_detail_single($current_employer_id);
    ...
    if($type_of_leave==1){
        ...
        $apply_leave  = $this->userModel->update_salary_cl($current_employer_id,$total_cl);
        // $apply_leave  = $this->userModel->apply_leave($type_of_leave,$start_date,$end_date,$number_of_days);
        // }
    }elseif($type_of_leave==3){
        // for sick leave
        // if($number_of_sl>=$number_of_days){
        $total_sl = $number_of_sl-$number_of_days;
        $apply_leave  = $this->userModel->update_salary_sl($current_employer_id,$total_sl);
        // $apply_leave  = $this->userModel->apply_leave($type_of_leave,$start_date,$end_date,$number_of_days);
        // }
        ...
        $apply_leave  = $this->userModel->update_salary_el($current_employer_id,$total_el);
        // $apply_leave  = $this->userModel->apply_leave($type_of_leave,$start_date,$end_date,$number_of_days);
        // }
    }else{
        ...
        // for od
        $apply_leave  = $this->userModel->update_salary_od($current_employer_id,$total_od);
        // $apply_leave  = $this->userModel->apply_leave($type_of_leave,$start_date,$end_date,$number_of_days);
        // }
       
    }
    $status_update = $this->adminModel->update_leave_status($id, $status);
}elseif($status == '4'){
$delete_leave = $this->userModel->delete_leave($id);
if($type_of_leave==1){
    $total_cl = $number_of_cl+$number_of_days;
   ...
   public function test(){
    ...
    {
        $employee_detail = $this->adminModel->get_employee_detail($id);
        $data = [
    ...
    {
        $upload = $this->adminModel->upload_leaves();
        if ($upload) {
            $_SESSION['success'] = 'Uploaded';
            redirect('hr/leaves');
        } else {
